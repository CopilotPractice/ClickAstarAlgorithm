...
std::vector<Node*> AStar::FindPath(Node* startNode, Node* goalNode, DemoLevel* refLevel, int (*wallsPos)[100])
{
	openList.clear();
	closedList.clear();
	...
	while (!openList.empty())
	{
		...
		refLevel->closedList.emplace_back(currentNode);
...
bool AStar::IsInRange(int x, int y)
{
	...
	if (x < 0 || x >= Engine::Get().ScreenSize().x || y < 0 || y >= Engine::Get().ScreenSize().y)
	{
		return false;
	}
